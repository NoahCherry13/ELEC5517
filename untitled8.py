...
def _lazy_import_mininet():
    ...
    try:
        ...
        raise RuntimeError('Failed to import Mininet modules: {}'.format(e))
...
def onos_rest(ctrl_ip, path, method='GET', data=None, user='onos', password='rocks', dry_run=False):
    ...
    url = "http://{}:8181{}".format(ctrl_ip, path)
    if dry_run:
        info("[DRY-RUN] REST {} {}\n".format(method, url))
        return 0, '{}'
...
def onos_activate_core_apps(ctrl_ip, user='onos', password='rocks', dry_run=False):
    ...
    for app in apps:
        code, _ = onos_rest(
            ctrl_ip,
            "/onos/v1/applications/{}/active".format(app),
            method='POST', user=user, password=password, dry_run=dry_run
        )
        info("ONOS app activate {}: HTTP {}\n".format(app, code))
...
def set_host_ips(host_map):
    ...
    for leaf, hosts in host_map.items():
        ...
        ip1 = "{}{}{}".format(base, start, "/" + str(mask))
        ip2 = "{}{}{}".format(base, start + 1, "/" + str(mask))
        if len(hosts) >= 1:
            hosts[0].setIP(ip1)
        if len(hosts) >= 2:
            hosts[1].setIP(ip2)
...
def tag_vlan_on_leaf_access(leaf, leaf_to_host_links, vlan_id, dry_run=False):
    for link in leaf_to_host_links:
        ...
        cmd = "ovs-vsctl set port {} tag={}".format(port_name, vlan_id)
        if dry_run:
            info("[DRY-RUN] {}\n".format(cmd))
        else:
            leaf.cmd(cmd)
...
def install_acl_isolation(leaf_name, deny_cidrs, dry_run=False):
    for cidr in deny_cidrs:
        cmd_ip = "ovs-ofctl -O OpenFlow13 add-flow {} priority=40000,ip,nw_dst={},actions=drop".format(leaf_name, cidr)
        cmd_arp = "ovs-ofctl -O OpenFlow13 add-flow {} priority=40000,arp,arp_tpa={},actions=drop".format(leaf_name, cidr)
        info("ACL on {}: deny {}\n".format(leaf_name, cidr))
        if dry_run:
            info("[DRY-RUN] {}\n".format(cmd_ip))
            info("[DRY-RUN] {}\n".format(cmd_arp))
        else:
...
def build_topology(ctrl_ip, ctrl_port, enable_vlan=False, enable_qos=False, program_onos=False, onos_user='onos', onos_pass='rocks', dry_run=False):
    ...
    if dry_run:
        ...
        info("Will connect to ONOS at {}:{} (if --program_onos).\n".format(ctrl_ip, ctrl_port))
        if enable_vlan:
        ...
        if program_onos:
            ...
            for pair in [("h1","h2"),("h3","h4"),("h5","h6"),("h7","h8")]:
                info("[DRY-RUN] Would post HostToHostIntent for {}↔{}\n".format(pair[0], pair[1]))
        info("\n=== Summary (dry‑run) ===\n")
...
def run_tests():
    """Pure‑python smoke tests that do NOT import Mininet.
    Run these anywhere (Windows/macOS/Linux) to validate helpers and dry‑run paths.
    """
    class _FakeHost:
    ...
    # REST dry-run should not perform network I/O and should return code 0
    code, _ = onos_rest('1.2.3.4', '/onos/v1/foo', method='GET', dry_run=True)
    assert code == 0, 'dry-run REST should return code 0'

    # QoS/VLAN/ACL helpers in dry-run should not shell out
    class _FakeLeaf:
        name = 'lX'
        def cmd(self, *_):
